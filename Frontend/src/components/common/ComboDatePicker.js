...
class ComboDatePicker extends Component {
	// ----- Main methods ----- //
	...
	render() {
		...
		console.log("mmmmmm",this.props.dia);
		var selects = {
			'd': (<ComboDatePicker.MySelect attrs={this.attrsDate}
																			handleSelectDay={this.props.handleSelectDay}
																			fechaID={this.props.dia}
																			type="date" model={this.model} items={dateList} placeholder={this.placeHolders[2]}
																			ref={(c) => this._date = c} onChange={this.updateModel.bind(this)} />),
			'm': (<ComboDatePicker.MySelect attrs={this.attrsMonth}
																			handleSelectMont={this.props.handleSelectMont}
																			mesID={this.props.mes}
																			type="month" model={this.model} items={monthList} placeholder={this.placeHolders[1]}
																			ref={(c) => this._month = c} onChange={this.updateModel.bind(this)} />),
			'y': (<ComboDatePicker.MySelect attrs={this.attrsYear}
																			handleSelectYear={this.props.handleSelectYear}
																			anioID={this.props.anio}
																			type="year" model={this.model} items={this.yearList} placeholder={this.placeHolders[0]}
																			ref={(c) => this._year = c} onChange={this.updateModel.bind(this)} />)
		}
		...
		return (
			<Col xs={4} md={4}>
				<h6>{Labels.Busqueda.de}</h6>
				<Col sm={4} md={4} className={'no-padding'}>
	...
	getDateList() {
		...
		let fechaSel = new Date(this.props.anio, this.props.mes, this.props.dia);
		if(fechaSel != null && fechaSel.getMonth() == this.minDate.getMonth() && fechaSel.getFullYear() == this.minDate.getFullYear()) {
		...
		var end = fechaSel != null? ComboDatePicker.maxDate(fechaSel.getMonth()+1, fechaSel.getFullYear()) : 31;
		if(fechaSel != null && fechaSel.getMonth() == this.maxDate.getMonth() && fechaSel.getFullYear() == this.maxDate.getFullYear()) {
	...
	getMonthList() {
		...
		var start = fechaSel != null && fechaSel.getFullYear() == this.minDate.getFullYear()? this.minDate.getMonth() : 0;
		var end = fechaSel != null && fechaSel.getFullYear() == this.maxDate.getFullYear()? this.maxDate.getMonth() : 11;
...
ComboDatePicker.MySelect = class extends React.Component {
	...
	constructor(props) {
		...
		this.state.handleSelectDay = props.handleSelectDay
		this.state.handleSelectMont = props.handleSelectMont
		this.state.handleSelectYear = props.handleSelectYear

		this.anioID = props.anioID
		this.mesID = props.mesID
	...
	render() {
		...
		if(this.value && this.state.items) {
			...
			if(min > max) { let auxi = min; min = max; max = auxi; }
		...
		for(var i=0; i<this.props.items.length; i++) {
			options.push({value: this.props.items[i].value, label: this.props.items[i].name}
			);
		}
		...
		if(this.placeholder) {
			options.unshift(
				<option value="" {...this.value? {disabled: true} : {}}>{this.placeholder}</option>
			);
		} else {
			if(!this.value) { options.unshift(<option value="" />); }
		}

		if(this.state.type == 'd' || this.state.type == 'date'){
			return (<SelectInput
				value={this.props.fechaID}
				placeholder={'Día'}
				onChange={(v)=>{this.state.handleSelectDay(v,'fechaID'),this.handleChange}}
				options={options} />);

		}

		if(this.state.type == 'm' || this.state.type == 'month'){
			return (<SelectInput
				{...this.attributes}
				value={this.props.mesID}
				placeholder={'Mes'}
				onChange={(v)=>{this.state.handleSelectMont(v,'mesID'),this.handleChange}}
				options={options} />);
		}

		if(this.state.type == 'y' || this.state.type == 'year'){
			return (<SelectInput
				{...this.attributes}
				value={this.props.anioID}
				placeholder={'Año'}
				onChange={(v)=>{this.state.handleSelectYear(v,'anioID'),this.handleChange}}
				options={options} />);
		}
		// Return value.
		/*return (<SelectInput
												{...this.attributes}
												onChange={(v)=>{props.handleSelect(v,'fechaID')}}
												 options={options} />);*/
	}
...
const mapStateToProps = (state) => {
	return {
		information: state.InformationComponent.get('Information').toJS()
	}
}
export default connect(mapStateToProps)(ComboDatePicker,ComboDatePicker.MySelect)
