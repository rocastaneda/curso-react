...
class ComboDatePicker2 extends Component {
	// ----- Main methods ----- //
	...
	constructor(props) {
		...
		// Initialize model.
		console.log("oooooooooooo",props.timezone);
		this.model = ComboDatePicker2.parseDate(props.date, props.timezone);
		...
		this.minDate = ComboDatePicker2.parseDate(props.minDate, props.timezone);
		if(this.minDate == null) {
			var now = new Date();
			this.minDate = new Date(now.getFullYear()-100, now.getMonth(), now.getDate(),
				now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds());
		}
		this.maxDate = ComboDatePicker2.parseDate(props.maxDate, props.timezone);
	...
	render() {
		...
		var selects = {
			'd': (<ComboDatePicker2.MySelect attrs={this.attrsDate}
			...
			'm': (<ComboDatePicker2.MySelect attrs={this.attrsMonth}
			...
			'y': (<ComboDatePicker2.MySelect attrs={this.attrsYear}
	...
	getDateList() {
		...
		var start = 1;
		//console.log("DIA ", this.model,  "  ", this.minDate);
		...
		var end = this.model != null? ComboDatePicker2.maxDate(this.model.getMonth()+1, this.model.getFullYear()) : 31;
	...
	updateModel() {
		...
		if(ComboDatePicker2.isValidValue(date) && ComboDatePicker2.isValidValue(month) && ComboDatePicker2.isValidValue(year)) {
			// Validate max day of month.
			var maxDate = ComboDatePicker2.maxDate(month+1, year);
...
ComboDatePicker2.MySelect = class extends React.Component {
	/**
	 * Constructor function.
	 */
	constructor(props) {
	...
	render() {
		...
		for(var i=0; i<this.state.items.length; i++) {
			options.push(
				<option value={this.state.items[i].value}
		...
		// Return value.
		return <select onChange={this.handleChange} {...this.attributes}>{options}</select>;
	}
...
export default connect(mapStateToProps)(ComboDatePicker2,ComboDatePicker2.MySelect)
