...
const TextInput = ({error, noupper, lower, name, label, integerDecimalObj, subLabel, onChange, onKeyDown, placeholder, readOnly, required, value, disabled, maxLength, num, float, alpha, alphanum, alphaquestion, email, rfc, autoFocus, facebook, floatValues, style}) => {
	...
	let handleChange = (e) => {
		...
		if (alpha) {
			...
			if (!ival.includes('.') && ival.length > integerDecimalObj.int)
				ival = ival.substr(0, integerDecimalObj.int)

			if (ival.match(/^[0-9]{0,10}[.]?[0-9]{0,2}$/) === null && ival.length >= 2) {
				ival = ival.split('.')
				ival = `${ival[0].substr(0, integerDecimalObj.int)}.${ival[1].substr(0, integerDecimalObj.dec)}`
			}
